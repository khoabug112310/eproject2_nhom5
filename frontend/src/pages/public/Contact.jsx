...
import { cmsAPI, profilesAPI } from '../../services/api';
...
export default function Contact() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      if (token) {
        try {
          const res = await profilesAPI.getMyPatientProfile();
          ...
          console.error('Error fetching patient profile for contact autofill:', err);
        }
      }
    };
    fetchProfile();
  ...
  const faqItems = [
    { q: 'Is Hopsontai Clinic open on weekends?', a: 'Yes. We are open continuously from Monday to Sunday, 7:00 AM to 8:00 PM every day (including public holidays).' },
    { q: 'How do I book an appointment online?', a: 'You can use the Quick Booking widget on the Home page or the Doctors page, then enter your contact details and chosen department. Our care team will call to confirm within 15–30 minutes.' },
    { q: 'Does the clinic accept health insurance?', a: 'The clinic issues full financial invoices for private insurers (Bao Viet, Prudential, PVI, etc.). For state health insurance, we support in-network coverage in accordance with Department of Health regulations.' },
    { q: 'What if I need emergency care after hours?', a: 'The clinic has a 24/7 hotline for first-aid advice and ambulance dispatch in Ho Chi Minh City at 091-444-4444.' },
  ];
  ...
  async function handleSubmit(e) {
    ...
    if (!phoneRegex.test(phone) && !emailRegex.test(phone)) {
      setErrorMsg('Invalid contact info. Please enter a valid Vietnamese phone number or email address (e.g. 0912345678 or name@domain.com)');
      setLoading(false); return;
    }
    try {
      // Map front-end state keys to the Mongoose schema field names
      const payload = {
      ...
      if (emailRegex.test(phone.trim())) {
        payload.senderEmail = phone.trim();
      } else if (patientProfile?.email) {
        payload.senderEmail = patientProfile.email;
      }

      await cmsAPI.submitContactInquiry(payload);
      setSuccessMsg('Your message has been sent successfully! Thank you for your feedback.');
      setName('');
      setPhone('');
      setMessage('');
    } catch (err) {
      setErrorMsg(err?.response?.data?.message || 'Unable to send right now. Please try again later.');
    } finally { setLoading(false); }
  }

  const quickInfo = [
    { cls: 'cq-blue', label: 'Address', value: '123 Nguyen Trai, District 5, HCMC' },
    { cls: 'cq-green', label: 'Hotline', value: '091-444-4444 | 1900-1234' },
    { cls: 'cq-purple', label: 'Support email', value: 'contact@hopsontai.vn' },
    { cls: 'cq-orange', label: 'Working hours', value: 'Mon–Sun: 07:00–20:00' },
  ];
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', position: 'relative', zIndex: 1 }} className="fade-in">
          ...
          <p style={{ fontSize: '16px', color: 'rgba(255,255,255,0.85)', lineHeight: '1.6', maxWidth: '650px', margin: '0 auto' }}>
            Hopsontai General Clinic is always ready to listen and answer any questions to deliver the best possible medical care.
          </p>
        </div>
      </section>
          ...
          <div className="contact-left">
            ...
            <div className="contact-card-new">
              <div className="contact-card-hd">
                ...
                <h2 className="contact-card-hd-title">Frequently Asked Questions</h2>
              </div>
            ...
            <div className="contact-card-new">
              <div className="contact-card-hd">
                ...
                <h2 className="contact-card-hd-title">Clinic Location</h2>
              </div>
              <div className="cn-map">
                <iframe
                  title="Hopsontai Clinic location"
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{ margin: '6px 0 0 0', fontSize: '14px', color: 'rgba(255,255,255,0.8)', fontWeight: '400' }}>
                We are committed to responding within <strong style={{ color: 'rgba(255,255,255,1)' }}>24 working hours</strong>.
              </p>
            </div>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <label htmlFor="cn-phone" style={{
                    ...
                    Phone number or Email <span style={{ color: '#ef4444' }}>*</span>
                  </label>
                  <input
                    id="cn-phone"
                    type="text"
