...
// ==========================================
// CÁC ENDPOINT DÀNH RIÊNG CHO LUỒNG PATIENT (TỰ PHỤC VỤ)
// ==========================================

// 1. Lấy thông tin hồ sơ của chính bệnh nhân đang đăng nhập
const getMyPatientProfile = async (req, res) => {
  try {
    const userId = req.user.userId || req.user.id || req.user._id; // Hổ trợ cả token cũ và token mới

    // Tìm kiếm profile bệnh nhân gắn liền với User ID này
    const patient = await Patient.findOne({ userId }).lean();

    // Nếu chưa từng khởi tạo, trả về data: null để Frontend nhận diện và bật form tạo mới
    if (!patient) {
      const { success: ok } = require('../../utils/response');
      return ok(res, null, 'Tài khoản chưa khởi tạo hồ sơ bệnh nhân cá nhân');
    }

    const { success: ok } = require('../../utils/response');
    return ok(res, patient, 'Lấy hồ sơ bệnh nhân thành công');
  } catch (err) {
    console.error('getMyPatientProfile error', err);
    const { fail } = require('../../utils/response');
    return fail(res, 'Lỗi hệ thống khi tải hồ sơ bệnh nhân', 500, err.message);
  }
};

// 2. Người dùng tự khởi tạo hồ sơ bệnh nhân lần đầu tiên
const createMyPatientProfile = async (req, res) => {
  try {
    const userId = req.user.userId || req.user.id || req.user._id;
    const body = req.body;

    // Kiểm tra xem hồ sơ đã tồn tại hay chưa để tránh tạo trùng lắp dữ liệu
    const existingPatient = await Patient.findOne({ userId });
    if (existingPatient) {
      return res.status(400).json({ success: false, message: 'Hồ sơ bệnh nhân của tài khoản này đã tồn tại' });
    }

    // Kiểm tra các trường dữ liệu bắt buộc theo Schema
    if (!body.fullName || !body.dateOfBirth || !body.gender || !body.phoneNumber) {
      return res.status(400).json({ success: false, message: 'Vui lòng điền đầy đủ các thông tin bắt buộc (*)' });
    }

    // Tạo bản ghi mới gắn chặt với userId của tài khoản
    const newPatient = await Patient.create({
      userId,
      fullName: body.fullName,
      dateOfBirth: new Date(body.dateOfBirth),
      gender: body.gender,
      identityCard: body.identityCard || undefined,
      phoneNumber: body.phoneNumber,
      address: body.address || '',
      insuranceCode: body.insuranceCode || undefined,
      emergencyContact: body.emergencyContact || ''
    });

    const { success: ok } = require('../../utils/response');
    return ok(res, newPatient, 'Khởi tạo hồ sơ bệnh nhân thành công', 201);
  } catch (err) {
    console.error('createMyPatientProfile error', err);
    const { fail } = require('../../utils/response');
    return fail(res, 'Lỗi khi khởi tạo hồ sơ bệnh nhân', 500, err.message);
  }
};

// 3. Người dùng tự cập nhật thông tin hồ sơ bệnh nhân của mình
const updateMyPatientProfile = async (req, res) => {
  try {
    ...
    if (!patient) {
      return res.status(404).json({ success: false, message: 'Không tìm thấy hồ sơ bệnh nhân để cập nhật' });
    }

    // Tiến hành cập nhật dữ liệu nếu được gửi lên từ form
    if (body.fullName) patient.fullName = body.fullName;
    if (body.dateOfBirth) patient.dateOfBirth = new Date(body.dateOfBirth);
    if (body.gender) patient.gender = body.gender;
    ...
    if (body.phoneNumber) patient.phoneNumber = body.phoneNumber;
    if (body.address !== undefined) patient.address = body.address;
    if (body.insuranceCode !== undefined) patient.insuranceCode = body.insuranceCode || undefined;
    if (body.emergencyContact !== undefined) patient.emergencyContact = body.emergencyContact;

    await patient.save();

    const { success: ok } = require('../../utils/response');
    return ok(res, patient, 'Cập nhật hồ sơ bệnh nhân thành công');
  } catch (err) {
    console.error('updateMyPatientProfile error', err);
    const { fail } = require('../../utils/response');
    return fail(res, 'Lỗi khi cập nhật hồ sơ bệnh nhân', 500, err.message);
  }
};
module.exports = { getAllUsers, getUserById, updateUser, createDoctor, getPatients, getAdminStats,getMyPatientProfile,createMyPatientProfile,updateMyPatientProfile };
